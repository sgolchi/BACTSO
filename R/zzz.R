...
.onAttach <- function(...) {
  rstanarmLib <- dirname(system.file(package = "anRpackage"))
  pkgdesc <- suppressWarnings(utils::packageDescription("anRpackage", lib.loc = rstanarmLib))
  if (length(pkgdesc) > 1) {
    builddate <- gsub(';.*$', '', pkgdesc$Packaged)
    packageStartupMessage(paste("anRpackage (Version ", pkgdesc$Version, ", packaged: ", builddate, ")", sep = ""))
  }
